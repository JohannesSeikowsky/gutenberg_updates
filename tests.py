# Run complete pipeline for three test books to observe behavior at each step.
# Tests different Wikipedia scenarios: no article found, one article found, two articles found.
# Outputs detailed debug info to console and test_results.txt.
...
for idx, test_case in enumerate(test_cases, 1):
    log(f"=== Testing Pipeline for Book {test_case} ({idx}/{len(test_cases)}) ===\n")
    ...
    log(f"\n=== Pipeline Test Complete for Book {test_case} ===")
